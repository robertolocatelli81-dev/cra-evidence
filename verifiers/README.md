...
refuses duplicate keys, NaN/Infinity, floats (integral ones such as `10.0` included: `JSON.parse` would silently make them `10`), lone surrogates, nesting deeper than 512 and a ledger line whose content (terminator excluded) exceeds 64 MiB
...
`verifiers/differential.py` builds 66 fixtures with the Python library — intact (unsigned, signed, trusted, with a
...
66 cases, 0 divergences (70 with `CRA_ORACLE_BIG=1`, which adds four big-line ledgers: exactly 64 MiB of content accepted, one byte more refused, and two 65 MiB lines); ablations: a JS verifier that accepts any stored bytes → 1 divergence, the pre-fix Go verifier that skipped a non-string hash → 2 divergences, both caught.
