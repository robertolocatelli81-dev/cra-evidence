...
refuses duplicate keys, NaN/Infinity, floats (integral ones such as `10.0` included: `JSON.parse` would silently make them `10`), lone surrogates, nesting deeper than 512 and a ledger line above 64 MiB
...
`verifiers/differential.py` builds 55 fixtures with the Python library — intact (unsigned, signed, trusted, with a
...
55 cases, 0 divergences (57 with `CRA_ORACLE_BIG=1`, which adds two 65 MiB-line ledgers); ablations: a JS verifier that accepts any stored bytes → 1 divergence, the pre-fix Go verifier that skipped a non-string hash → 2 divergences, both caught.
